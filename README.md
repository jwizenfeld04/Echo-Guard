...
Echo Guard runs **entirely on your machine** — the embedding model, AST analysis, and all detection
happen locally via ONNX Runtime. No code is sent anywhere for analysis.
...
- All collection code is open source in [`echo_guard/upload.py`](echo_guard/upload.py)
- Full field-level schema in [`docs/FEEDBACK_SCHEMA.md`](docs/FEEDBACK_SCHEMA.md)

**No cloud dependencies for core functionality** — scanning, indexing, and detection
never require network access. Data sharing is optional and off by default for private repos.
...
- [x] **Consent-based feedback** — Three-tier data sharing with smart defaults, automatic uploads, transparency tools
