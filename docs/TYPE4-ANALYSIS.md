# Type-4 Clone Detection: Why 96% on GPTCloneBench but 0% on BigCloneBench?
...
| **Embedding score range** | **0.90-0.97** | **0.56-0.75** |
| **Above threshold (0.81)?** | Yes (100%) | No (0%) |
...
Echo Guard's Type-4 detection is calibrated for the **most common real-world scenario**: AI agents generating code that already exists in your codebase. This is the GPTCloneBench case — and we detect **96%** of those.
...
1. **Fine-tuning** UniXcoder on clone detection pairs (would improve BCB Type-4 significantly)
2. **Semantic understanding** beyond structural similarity (understanding what code *does*, not what it *looks like*)

For AI-assisted development, where duplicates are generated by the same LLM that produced similar code moments ago, the current detection is highly effective. For legacy codebase auditing where different developers independently wrote the same logic years apart, fine-tuning would be needed.
